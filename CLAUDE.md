...
- 표제지, 판권지를 실제 책처럼 둔다.
- **얇은 sticky 내비게이션(`.booknav`)을 상단에 둔다.** 브랜드명(#top) · 차례(#toc) · 부록(#appendix) · 도구함(tools.html) 네 개 링크만. 부록·도구 페이지에서 다시 돌아올 방법이 없으면 안 되므로, 모든 하위 페이지에도 같은 역할의 `.sitenav`(← 처음으로 / 도구함)를 넣는다.
...
- 최상단에 `.sitenav`(← 처음으로 / 도구함)를 둔다. 이미 자체 sticky 바(타이머·답보기 토글 등)가 있는 페이지는 그 바 안에 `.nav-back` 링크로 합친다.
