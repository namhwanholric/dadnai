...
이 폴더(`ai-study/`)에서 작업할 때 항상 이 문서를 먼저 읽는다.

> **위치 안내** — 이 사이트는 저장소 루트에 있다가 `ai-study/` 폴더로 옮겨졌다.
> 저장소 루트는 이제 웹소설 사이트(달빛서고)가 쓰고 있다. 저장소 전체 구조는 [../CLAUDE.md](../CLAUDE.md) 참고.
> **이 폴더에는 npm 의존성이나 빌드 도구를 추가하지 않는다.** 순수 HTML 그대로 배포된다.
...
GitHub Pages, `main` 브랜치. Source는 **GitHub Actions**다.

```
https://namhwanholric.github.io/dadnai/ai-study/
```

- 이 폴더는 빌드하지 않는다. 루트의 `npm run build`가 `scripts/copy-ai-study.mjs`로
  이 폴더를 `out/ai-study/`에 통째로 복사한다. **파일을 추가하면 자동으로 같이 올라간다.**
- `CLAUDE.md`와 `README.md`는 복사에서 제외된다(작업용 문서). 다른 파일을 제외하려면
  `scripts/copy-ai-study.mjs`의 `SKIP` 목록에 넣는다.
- 옛 주소(`/dadnai/파일명.html`)는 루트 `public/`의 리다이렉트 스텁이 받아서 여기로 보낸다.
- 커밋 메시지는 한국어로 간결하게. 예: `부록 B에 문자와 식 8문항 추가`
- 푸시 전 링크가 깨지지 않았는지 확인한다. 링크는 계속 `./파일명.html` 상대경로로 쓴다.
